...
def parse_args():
    ...
    # parser.add_argument('-r', '--reset', action='store_true', help='Reset Playlist')
...
def reset_playlist(args, m_svc, playlist):
    if args.dry_run:
        return
    ...
    items = m_svc.playlist_items(playlist["id"])
    if items:
        if args.reset:
            response = 'y'
        else:
            user_input = input(f"The playlist '{m_svc.playlist_name(playlist)}' is not empty. Do you want to reset it? [Default: n]: ").strip().lower()
            response = user_input[0] if user_input else "n"

        if response == 'y':
            m_svc.delete_songs_from_playlist(playlist, items)

def update_playlist(m_svc, args, playlist, song_list):
...
def playlist_name(m_svc, args, playlist):
    if args.dry_run:
        return "(dry-run) [No Playlist]"
    else:
        return m_svc.playlist_name(playlist)
...
def main():
    ...
    pl_name = playlist_name(m_svc, args, playlist)
    # reset_playlist(args, m_svc, playlist)

    print(f"Updating Playlist {pl_name}")
    update_playlist(m_svc, args, playlist, song_list)
    print(f"Playlist {pl_name} update complete.")
